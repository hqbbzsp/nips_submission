...
def toysubmission_generate(
    ...
    *,
    ...
    stopping_criteria = StoppingCriteriaList([MaxLengthCriteria(max_length=max_returned_tokens)])
    
    # generate up to a fixed number of tokens
    for _ in range(max_returned_tokens - T):
        # forward
        logits = model(input_ids=idx, return_dict=False)
        logits = logits[0][0][-1] / temperature
    ...
    #    idx = idx.index_copy(0, input_pos, idx_next)
        ...
        if idx_next == eos_id:
            return idx, logprob, top_logprob  # include the EOS token
